...
document.addEventListener('DOMContentLoaded', () => {
	...
	function renderExpenses(expenses) {
		...
		expenses.forEach(expense => {
            ...
            <td>${
							normalizedExpense.amount ? `${normalizedExpense.amount} ₽` : '0 ₽'
						}</td>
            <td>${formattedDate}</td>
        ...
        `
				...
				if (confirm('Вы уверены, что хотите удалить эту запись?')) {
					try {
						const response = await fetch(
							`${apiBaseUrl}/${normalizedExpense.id}`,
							{
								method: 'DELETE',
							}
						)
			...
			row.addEventListener('click', e => {
				if (!isSwiped && startX === undefined) {
					...
					openEditModal(normalizedExpense)
				}
			})
	...
	function showExpenseAnalysis(expenses, dateFrom, dateTo) {
		...
		document.querySelector(
			'#chartContainer h2'
		).textContent = `Анализ расходов с ${dateFrom} по ${dateTo}`
	...
	form.addEventListener('submit', async event => {
		...
		if (!expenseData.place || !expenseData.category || !expenseData.amount) {
			alert('Please fill in the required fields (location, category, amount)')
			return
		}
		...
		try {
			...
			if (!response.ok) throw new Error('Failed to add the record')
			...
			alert('Failed to add the record: ' + error.message)
		}
	})
        ...
        `
			...
			categoryRow.addEventListener('click', () => {
				...
				const existingDetails = document.querySelectorAll(
					`tr[data-parent-category="${category}"]`
				)
	...
	function openEditModal(expense) {
		...
		document.getElementById('editComment').value = expense.comment || ''

		// Инициализация flatpickr для поля даты
		if (!document.getElementById('editDate')._flatpickr) {
			flatpickr('#editDate', {
				enableTime: true,
				dateFormat: 'd.m.Y H:i',
				locale: 'ru',
				time_24hr: true,
				allowInput: true,
				defaultDate: expense.date ? new Date(expense.date) : new Date(),
			})
		} else {
			document
				.getElementById('editDate')
				._flatpickr.setDate(expense.date ? new Date(expense.date) : new Date())
		}
	...
	editForm.addEventListener('submit', async e => {
		...
		console.log('Отправляемые данные PATCH:', updateData)

		if (!updateData.place || !updateData.category || !updateData.amount) {
			alert('Please fill in the required fields (location, category, amount)')
			return
		}
		...
		try {
			...
			if (!response.ok) throw new Error('Error updating consumption')
	...
	searchForm.addEventListener('submit', async e => {
		...
		try {
			const response = await fetch(
				`${apiBaseUrl}/search?dateFrom=${dateFrom}&dateTo=${dateTo}`
			)
